...
static struct window find_window(const char *a, size_t a_len, const char *b, size_t b_len)
{
    ...
    const char *nl = s ? (const char *)memchr(a + (a_len - s), '\n', s) : NULL;
    if (!nl) {
        w.a_hi = a_len;
        w.b_hi = b_len;
        return w;
    ...
    for (int i = 0; i < WINDOW_SLACK_LINES && hi < a_len; i++) {
        const char *end = (const char *)memchr(a + hi, '\n', a_len - hi);
                        ...
                        const char *a_label, const char *b_label)
{
    ...
    if (a_win > (size_t)UINT32_MAX || b_win > (size_t)UINT32_MAX) {
        ...
        unsigned char *a_changed = (unsigned char *)xcalloc(a.count + 1, 1);
        unsigned char *b_changed = (unsigned char *)xcalloc(b.count + 1, 1);
        long *kv = (long *)xmalloc(2 * (2 * (size_t)REGION_STEPS_MAX + 3) * sizeof(*kv));
        struct dctx ctx = {
            .a = &a,
            .b = &b,
            .a_changed = a_changed,
