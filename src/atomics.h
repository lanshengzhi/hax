...
template <typename T>
inline void atomic_init(T *p, typename T::value_type v)
{
    p->store(v, std::memory_order_relaxed);
}

template <typename T>
inline auto atomic_load(const T *p)
{
    return p->load();
}

template <typename T>
inline auto atomic_load_explicit(const T *p, std::memory_order order)
{
    return p->load(order);
}

template <typename T>
inline void atomic_store(T *p, typename T::value_type v)
{
    p->store(v);
}

template <typename T>
inline auto atomic_exchange(T *p, typename T::value_type v)
{
    return p->exchange(v);
}

template <typename T>
inline auto atomic_fetch_add(T *p, typename T::value_type v)
{
    return p->fetch_add(v);
}

template <typename T>
inline auto atomic_fetch_add_explicit(T *p, typename T::value_type v,
