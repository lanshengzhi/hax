...
{
    struct traced_sse_callback *traced = (struct traced_sse_callback *)user;
...
static const char **copy_headers_with(const char *const *headers, const char *extra_header)
{
    ...
    const char **copy = (const char **)xmalloc((count + 2) * sizeof(*copy));
...
static size_t capture_sse_header(char *data, size_t size, size_t count, void *user)
{
    ...
    struct sse_request_state *state = (struct sse_request_state *)user;
...
{
    struct buffered_request_state *state = (struct buffered_request_state *)user;
    size_t len = size * count;
