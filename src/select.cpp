...
{
    (void)job;
    struct availability_job_ctx *ctx = (struct availability_job_ctx *)arg;
    char *body = NULL;
...
{
    struct bg_job **jobs = (struct bg_job **)xcalloc(count, sizeof(*jobs));
    for (size_t i = 0; i < count; i++) {
        results[i].available = 1; /* default if a network worker cannot be spawned */
        results[i].reason = NULL;
        struct availability_job_ctx *ctx = (struct availability_job_ctx *)xcalloc(1, sizeof(*ctx));
...
struct provider *provider_autoselect(void)
{
    ...
    if (default_factory && factory_available(default_factory, NULL)) {
        struct provider *provider = default_factory->create(default_factory->id);
        if (provider) {
    ...
    for (size_t i = 0; i < factory_count && !provider; i++) {
        ...
        provider = factories[i]->create(factories[i]->id);
        if (provider)
...
{
    const struct model_info *left_model = (const struct model_info *)left;
    const struct model_info *right_model = (const struct model_info *)right;
                                                     ...
                                                     const char *current_model)
{
    ...
    struct picker_item *items = (struct picker_item *)xcalloc(model_count, sizeof(*items));
    char **descriptions = (char **)xcalloc(model_count, sizeof(*descriptions));
    size_t initial = 0;
    for (size_t i = 0; i < model_count; i++) {
                                              ...
                                              int announce_unavailable)
{
    ...
    struct picker_item *items = (struct picker_item *)xcalloc(levels.count + 1, sizeof(*items));
...
static struct provider_pick_result choose_provider_factory(const char *current_provider_id)
{
    ...
    struct picker_item *items = (struct picker_item *)xcalloc(factory_count, sizeof(*items));
    char **descriptions = (char **)xcalloc(factory_count, sizeof(*descriptions));
    size_t initial = 0;
    for (size_t i = 0; i < factory_count; i++) {
...
static char *choose_preset_name(char **names, size_t count)
{
    ...
    struct picker_item *items = (struct picker_item *)xcalloc(count, sizeof(*items));
    char **details = (char **)xcalloc(count, sizeof(*details));
...
static struct value_pick_result choose_tint(const char *current_tint)
{
    ...
    struct picker_item *items = (struct picker_item *)xcalloc(value_count + 1, sizeof(*items));
    items[0].label = "none";
...
void select_preset_save(struct agent_state *state, const char *argument)
{
    ...
    char *name = (char *)xmalloc(name_length + 1);
                            ...
                            const char *effort, const char *preset)
{
    ...
    struct provider *candidate = factory ? factory->create(factory->id) : NULL;
    sync_constructor_diagnostics(state, diagnostics_before);
    if (!candidate) {
        if (!factory)
...
static void pick_setting_choice(struct agent_state *state, const struct config_setting *setting)
{
    ...
    struct picker_item *items = (struct picker_item *)xcalloc(item_count, sizeof(*items));
    items[0].label = "default";
    items[0].description =
...
static const struct config_setting *choose_config_setting(void)
{
    ...
    struct picker_item *items = (struct picker_item *)xcalloc(shown_count, sizeof(*items));
    char **details = (char **)xmalloc(shown_count * sizeof(*details));
    char **descriptions = (char **)xcalloc(shown_count, sizeof(*descriptions));
