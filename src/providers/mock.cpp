...
static struct stream_usage parse_usage(const char *spec)
{
    ...
    while (*spec) {
        ...
        const char *separator = (const char *)memchr(spec, '=', (size_t)(token_end - spec));
        if (!separator) {
            spec = token_end;
            continue;
        }
...
{
    char *decoded = (char *)xmalloc(strlen(text) + 1);
    char *output = decoded;
    while (*text) {
        if (text[0] == '\\' && text[1] == 'n') {
            *output++ = '\n';
...
{
    struct mock_provider *mock = (struct mock_provider *)xcalloc(1, sizeof(*mock));
    mock->base.name = "mock";
    mock->base.id = id;
