...
{
    struct paint_context *context = (struct paint_context *)user;
...
{
    struct row_widths *widths = (struct row_widths *)user;
...
{
    struct buf *frame = (struct buf *)user;
...
static void complete_modal(struct input *in, size_t start, size_t end)
{
    ...
    char *token = (char *)xmalloc(token_len + 1);
...
{
    struct buf *query = (struct buf *)user;
...
static size_t history_file_load(struct input *in, const char *path)
{
    ...
    if (f) {
        ...
        char *line = (char *)xmalloc(HISTORY_RECORD_MAX);
        for (;;) {
            size_t n = 0;
            int c, overflow = 0;
