...
char *session_picker_run(const char *cwd, const char *exclude_path, int *picker_opened)
{
    ...
    struct picker_item *items = (struct picker_item *)xcalloc(picker_count, sizeof(*items));
    char **details = (char **)xmalloc(picker_count * sizeof(*details));
    char **provenances = (char **)xmalloc(picker_count * sizeof(*provenances));
