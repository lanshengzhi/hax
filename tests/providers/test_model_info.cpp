...
static void test_openrouter_effort_levels(void)
{
    ...
    const char *entry = "{\"id\":\"x-ai/grok-4.5\",\"reasoning\":{\"mandatory\":true,"
...
static void test_openrouter_effort_metadata_states(void)
{
    ...
    const char *plain_model =
        "{\"id\":\"vendor/plain\",\"supported_parameters\":[\"tools\",\"stop\"]}";
    ...
    const char *reasoning_toggle =
        "{\"id\":\"moonshotai/kimi\",\"supported_parameters\":[\"reasoning\"],"
    ...
    const char *router = "{\"id\":\"openrouter/auto\",\"supported_parameters\":"
    ...
    const char *empty_levels = "{\"id\":\"vendor/m\",\"reasoning\":{\"supported_efforts\":[]}}";
...
static void test_codex_model_capabilities(void)
{
    ...
    EXPECT(!codex_model_is_hidden(model_json));
...
{
    const char *hidden = "{\"slug\":\"codex-auto-review\",\"visibility\":\"hide\"}";
    EXPECT(codex_model_is_hidden(hidden));

    const char *visible = "{\"slug\":\"x\"}";
...
{
    struct effort_set efforts = {0};
    const char *entry = "{\"slug\":\"gpt-5.6-sol\",\"supported_reasoning_levels\":["
...
{
    struct effort_set efforts = {0};
    const char *entry = "{\"supported_reasoning_levels\":[\"low\",\"medium\",\"high\"]}";
    codex_parse_model_efforts(entry, &efforts);
...
static void test_codex_effort_metadata_states(void)
{
    ...
    const char *without_levels = "{\"slug\":\"x\",\"context_window\":272000}";
    codex_parse_model_efforts(without_levels, &absent);
    EXPECT(!absent.known && absent.count == 0);

    struct effort_set empty = {0};
    const char *with_empty_levels = "{\"slug\":\"x\",\"supported_reasoning_levels\":[]}";
